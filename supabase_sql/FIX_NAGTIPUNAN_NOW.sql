-- Fix Nagtipunan barangays - set to 0 and mark as energized
-- Run this in Supabase SQL Editor

-- First, check what the actual municipality name is
SELECT DISTINCT municipality FROM public.barangays WHERE municipality LIKE '%NAGTIPUNAN%';

-- Get the barangay IDs we need to update
...
-- Using LIKE to match the municipality name regardless of exact format
WITH target AS (
...
SELECT '✅ Check overrides were created:' as status;
...
SELECT '✅ Check household_status for Nagtipunan:' as status;
SELECT * FROM public.household_status WHERE municipality LIKE '%NAGTIPUNAN%';

SELECT '✅ Check municipality_status for Nagtipunan (should show 16 total, 13 energized):' as status;
SELECT * FROM public.municipality_status WHERE municipality LIKE '%NAGTIPUNAN%';

SELECT '✅ Check barangay details:' as status;
